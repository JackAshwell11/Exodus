...
void run_system(ecs::Registry& registry, Camera& camera, const float delta_time) {
  ...
  } else if constexpr (std::is_invocable_v<decltype(System), ecs::Registry&, const Camera&>) {
    System(registry, static_cast<const Camera&>(camera));
  } else {
...
void run_systems(ecs::Registry& registry, Camera& camera, const float delta_time) {
  (run_system<Systems>(registry, camera, delta_time), ...);
}
}  // namespace
...
void Engine::fixed_update(const float delta_time) {
  run_systems<ecs::systems::input_system, ecs::systems::movement_system>(*registry_, camera_, delta_time);
}

void Engine::render(const float delta_time) {
  run_systems<ecs::systems::sprite_render_system>(*registry_, camera_, delta_time);
}
}  // namespace exodus
